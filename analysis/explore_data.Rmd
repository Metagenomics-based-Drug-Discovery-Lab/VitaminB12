...
joined_data <- dplyr::full_join(Final_Dataset, medications, by =c("patient_no", "clinic_date"))
dim(joined_data)
...
paste0("categories are ", unique(joined_data$metformin_use))
```
...
```{r}
joined_data <-  joined_data %>% 
  select(-c(hospital_name_x, enterprise_number, exam_detail_item)) %>% 
  filter(!is.na(gender_x))
...
joined_data %>% 
    mutate(gender = factor(gender_x)) %>% 
    ggboxplot(x = "gender", y = "exam_result",
...
model <- lm(exam_result ~ age+ gender_x+bmi,joined_data)
